...
import IntelligenceInlineCard from "../constructor/intelligence/IntelligenceInlineCard";
    ...
    <>
      ...
      <main className="cp-page">
        ...
        <section className="cp-workspace">
          <aside className="cp-config">
            <div className="cp-steps">
              {[["size", "Размеры"], ["fill", "Наполнение"], ["materials", "Материалы"]].map(([id, label]) => (
            ...
            {activeStep === "size" && (
              <div className="cp-card">
                <div className="cp-card-head"><span>01</span><h2>Размеры и секции</h2></div>
                  ...
                  {Object.keys(DIMENSION_LIMITS).map((key) => {
                    ...
                    return (
                      <div className="cp-dimension" key={key}>
                        <div><strong>{item.label}, мм</strong><small>{item.min}–{item.max}</small></div>
                        <div className="cp-counter">
                          <button type="button" onClick={() => stepDimension(key, -1)}>−</button>
                          <input type="number" value={draft[key]} onChange={(event) => setDraft((prev) => ({ ...prev, [key]: event.target.value }))} onBlur={(event) => commitDimension(key, event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} />
                          <button type="button" onClick={() => stepDimension(key, 1)}>+</button>
                        </div>
                      </div>
                    );
                ...
                <div className="cp-section-count">
                  <div><strong>Количество секций</strong><small>от 1 до 6, ширина распределяется автоматически</small></div>
                ...
                <div className="cp-section-widths">
                  <div className="cp-section-widths-head"><strong>Секции шкафа</strong><small>сейчас ширина распределяется автоматически</small></div>
                  <div className="cp-section-width-grid" style={{ gridTemplateColumns: `repeat(${sectionCount}, minmax(44px, 1fr))` }}>
                    {config.sections.map((section, index) => (
                      <button key={section.id} type="button" className={section.id === activeSection?.id ? "active" : ""} onClick={() => selectSection(section.id)}>
                        <span>{index + 1}</span><b>{autoSectionWidth} мм</b><i />
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {activeStep === "fill" && activeSection && (
              <div className="cp-card">
                <div className="cp-card-head"><span>02</span><h2>Наполнение секции</h2></div>
                <div className="cp-section-tabs">
                  {config.sections.map((section, index) => (
                    <button key={section.id} type="button" className={section.id === activeSection.id ? "active" : ""} onClick={() => setActiveSectionId(section.id)}>{index + 1}</button>
                  ))}
                </div>
                {activeSectionIsEmpty ? <div className="cp-empty-state"><strong>Секция пока пустая</strong><small>Выберите готовый пресет или добавьте полку, ящик, штангу вручную.</small></div> : null}
                <div className="cp-presets">
                  {FILL_PRESETS.map((preset) => <button type="button" key={preset.id} onClick={() => applyPreset(preset)}><strong>{preset.label}</strong><small>{preset.desc}</small></button>)}
                </div>
                ...
                {showHandles ? <div className="cp-handle-options">{handleOptions.map((option) => <button key={option.id} type="button" className={config.facade.handleVariant === option.id ? "active" : ""} onClick={() => setHandleVariant(option.id)}>{option.label}</button>)}</div> : null}
              </div>
            )}

            {activeStep === "materials" && (
              <div className="cp-card">
                <div className="cp-card-head"><span>03</span><h2>Материалы</h2></div>
        ...
        <div className={`cp-mobile-bar ${pricePulse ? "is-price-updated" : ""}`}>
          <div className="cp-mobile-bar-price"><span>Стоимость</span><strong>{price.toLocaleString("ru-RU")} ₽</strong><small>{config.dimensions.width} × {config.dimensions.height} × {config.dimensions.depth} мм</small></div>
          <div className="cp-mobile-bar-actions"><button type="button" onClick={saveDraft} aria-label="Сохранить">♡</button><button type="button" onClick={() => navigate("/account/order")}>В корзину</button></div>
        </div>
      </main>

      {drawerType ? <MaterialDrawer type={drawerType} config={config} onClose={() => setDrawerType(null)} onBody={setBodyMaterial} onFacade={setFacadeMaterial} onHardware={setHardwareBrand} /> : null}
    </>
  );
}

function FillCounter({ label, value, onMinus, onPlus }) {
  return <div className="cp-fill-counter"><span>{label}</span><div><button type="button" onClick={onMinus}>−</button><b>{value}</b><button type="button" onClick={onPlus}>+</button></div></div>;
}

function MaterialSelect({ title, name, color, onClick }) {
  return <button type="button" className="cp-material-select" onClick={onClick}>{color ? <span style={{ background: color }} /> : <i />}<div><small>{title}</small><strong>{name}</strong></div><em>Изменить</em></button>;
}
...
function MaterialDrawer({ type, config, onClose, onBody, onFacade, onHardware }) {
  ...
  return <><div className="cp-drawer-overlay" onClick={onClose} /><div className="cp-drawer"><div className="cp-drawer-head"><h3>{title}</h3><button type="button" onClick={onClose}>✕</button></div><div className="cp-drawer-grid">{options.map((option) => <button key={option.id} type="button" className={active(option) ? "active" : ""} onClick={() => select(option)}>{type !== "hardware" ? <span style={{ background: option.color }} /> : null}<strong>{option.name || option.label}</strong><small>{option.subtitle || "петли и направляющие"}</small></button>)}</div></div></>;
}
