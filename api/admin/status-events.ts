...
import { listAdminStatusEvents } from '../_shared/admin-orders'
...
export default async function handler(req: ServerlessRequest, res: ServerlessResponse) {
  ...
  try {
    ...
    const events = await listAdminStatusEvents(limit)
    return res.status(200).json({ ok: true, events })
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    logEvent('error', 'admin.status_events_list_failed', { requestId, reason: message.slice(0, 300) })
    return res.status(500).json({ ok: false, message: 'Unable to load status events' })
  }
}
