-- Epic I-1 / Customer Change Request API Foundation
-- Formal customer requests to change submitted orders (not direct order editing).
...
comment on table public.order_change_requests is 'Customer-submitted change requests for owned orders';
comment on column public.order_change_requests.request_type is 'Controlled change request category';
comment on column public.order_change_requests.message is 'Customer free-text change description';
comment on column public.order_change_requests.status is 'Workflow status; I-1 uses submitted only';
